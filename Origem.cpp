...
TCHAR szTIP[64] = TEXT("Click to generate a temp folder");
char szClassName[] = "ColdFox";
...
void generateFolder() {
	...
	ss << getenv("TEMP") << "\\" << buff;
	std::string FolderSS = ss.str();
	std::wstring wide = std::wstring(FolderSS.begin(), FolderSS.end());


	CreateFolder(FolderSS.c_str());
	ShellExecute(NULL, NULL, FolderSS.c_str(), NULL, NULL, SW_SHOWNORMAL);
}
